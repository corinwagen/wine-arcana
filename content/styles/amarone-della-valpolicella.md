---
title: Amarone della Valpolicella
aliases:
  - Amarone
---

# Amarone della Valpolicella

Amarone della Valpolicella is the dry red branch of [Valpolicella's](../regions/valpolicella.md)
*[appassimento](../concepts/appassimento.md)* tradition. Its grapes are partially dried after harvest, then
fermented rather than vinified fresh. Water loss gives the must more sugar and
a higher ratio of skins to juice, so fermentation can produce substantial
alcohol, extract, and body. “Dry” distinguishes Amarone above all from sweet
Recioto della Valpolicella; it does not mean that every bottle contains almost
no [residual sugar](../concepts/residual-sugar.md).

Amarone is both a method-defined style and a protected denomination of origin.
The current specification controls its production zone, grape framework,
drying, analytical limits, and minimum maturation. It leaves important choices
to the producer, however. Site, fruit condition, drying pace, fermentation,
extraction, remaining sugar, and maturation can yield markedly different wines
within the same legal name.

## How it is made

The current rules require 45–95% [Corvina](../grapes/corvina.md) and/or
[Corvinone](../grapes/corvinone.md)
combined, plus 5–30% [Rondinella](../grapes/rondinella.md). Up to 25% in total may come from other
permitted red varieties, subject to category and per-variety limits. Corvinone
is a separate variety, and the combined range means that Corvina itself is not
legally indispensable if Corvinone fills that share.[^1]

Sound bunches intended for Amarone are selected at harvest and typically laid
in a single layer in crates or on racks in ventilated drying rooms called
*fruttai*. Over the following weeks, evaporation concentrates fermentable
sugar and other dissolved material while the skin-to-juice ratio rises. The
berry remains metabolically active: acids, phenolic compounds, and aroma
precursors may change rather than simply increasing in proportion to water
loss. Temperature, humidity, airflow, berry condition, and time therefore
alter the raw material. In a controlled Corvina experiment, natural drying
took about twice as long as strong forced-air drying to reach the same 30%
weight loss, and the two treatments produced different patterns of gene
expression and metabolite accumulation.[^2]

Grapes must dry
in suitable premises; environmental control is allowed at temperatures
comparable to traditional practice, but heat-assisted dehumidification is not.
Vinification cannot normally begin before 1 December, although the Veneto
region may authorize an earlier start when conditions require it. After
drying, the grapes must reach at least 14% potential alcohol, and finished-wine
yield cannot exceed 40% of the grape weight.[^1]

The dried grapes are crushed and fermented on their skins during winter.
Their concentrated sugar raises potential alcohol but also places yeast under
osmotic stress; cold fruit and the rising alcohol level can make the last part
of fermentation slow or difficult. Temperature, nutrients, yeast population,
and extraction must therefore be managed together. Fermenting most of the
sugar converts the concentrated must into Amarone's relatively dry, alcoholic
form. Many wines then undergo [malolactic
fermentation](../concepts/malolactic-fermentation.md).[^3]

Before release, Amarone must mature for at least two years, counted from 1
January after the harvest. *Riserva* requires at least four years from 1
November of the harvest year. The specification does not prescribe one barrel
size or wood regime for this period. Producers commonly use wood, but large
casks, smaller barrels, blending, and further bottle age can lead in different
directions. Newer or smaller wood can contribute more wood-derived flavour and
[tannin](../concepts/tannin.md), while larger or older vessels generally make [oak maturation](../concepts/oak-maturation.md)
less overt.

## Amarone, Recioto, and Ripasso

Amarone and Recioto begin with the same required grape ranges and closely
parallel appassimento rules. Their decisive divergence comes during
fermentation. Recioto retains enough grape sugar to be legally sweet; its
specification requires at least 2.8% potential alcohol to remain after
fermentation.[^4] Amarone ferments much farther. At release it must contain at
least 14% actual alcohol, while total residual sugar is capped at 9 g/L at 14%
alcohol and may rise incrementally as alcohol increases.[^1] Amarone can
therefore taste dry or gently sweet depending on alcohol, [acidity](../concepts/acidity.md), fruit,
serving conditions, and where its sugar falls below that ceiling.

Valpolicella Ripasso follows another route. It starts as eligible Valpolicella
wine made from grapes vinified without Amarone's prolonged pre-fermentation
drying. That wine is refermented once on the pomace left after Amarone and/or
Recioto has been drawn off. Remaining wine, sugar, skins, and seeds can add
alcohol, colour, tannin, and extract, depending on the base wine, pomace, and
contact. Ripasso receives a second fermentation from material that has already
made
an appassimento wine.[^4]

## Character and variation

Water loss
can intensify sound, balanced fruit, but it can also magnify overripeness,
[oxidation](../concepts/oxidation.md), rot, or difficult fermentation. More drying may increase potential
alcohol and extract while reducing juice yield and making a clean dry finish
harder to achieve. A slower process gives the berry more time for metabolic
change, yet slow drying is useful only while the fruit remains healthy.

Vineyard and cellar choices remain visible through the method. Grapes from
different elevations and exposures enter drying with different acidity and
maturity. Corvina, Corvinone, Rondinella, and minor varieties contribute
different pigment, tannin, and aroma material. Some producers emphasize
acidity and restrained extraction; others combine riper fruit, a higher level
of remaining sugar, and more assertive wood for a broader impression. Bottle
age can soften tannin and move the wine away from primary fruit, but longevity
depends on balance, sound fermentation, closure, and storage as well as
concentration.

[^1]: Ministero dell'agricoltura, della sovranità alimentare e delle foreste,
    production specification for Amarone della Valpolicella DOCG,
    consolidated 3 August 2023, articles 2, 4–6.
[^2]: Sara Zenoni et al., “Rapid dehydration of grape berries dampens the
    post-ripening transcriptomic program and the metabolite profile evolution,”
    *Horticulture Research* 7 (2020), article 141.
[^3]: Lanfranco Paronetto & Franco Dellaglio, “Amarone: A Modern Wine Coming
    from an Ancient Production Technology,” *Advances in Food and Nutrition
    Research* 63 (2011), pp. 285–306.
[^4]: Ministero dell'agricoltura, della sovranità alimentare e delle foreste,
    production specifications for Recioto della Valpolicella DOCG and
    Valpolicella Ripasso DOC, consolidated 3 August 2023, especially articles
    5–6.

## Related topics

- [Valpolicella](../regions/valpolicella.md)
- [Corvina](../grapes/corvina.md)
- [Oak maturation](../concepts/oak-maturation.md)

## Sources

- Ministero dell'agricoltura, della sovranità alimentare e delle foreste,
  [production specification for Amarone della Valpolicella
  DOCG](https://www.masaf.gov.it/flex/cm/pages/ServeBLOB.php/L/IT/IDPagina/20309),
  consolidated 3 August 2023, accessed 1 September 2026.
- Ministero dell'agricoltura, della sovranità alimentare e delle foreste,
  production specifications for [Recioto della Valpolicella
  DOCG](https://www.masaf.gov.it/flex/cm/pages/ServeBLOB.php/L/IT/IDPagina/20310)
  and [Valpolicella Ripasso
  DOC](https://www.masaf.gov.it/flex/cm/pages/ServeBLOB.php/L/IT/IDPagina/20313),
  consolidated 3 August 2023, accessed 1 September 2026.
- Lanfranco Paronetto & Franco Dellaglio, [“Amarone: A Modern Wine Coming from
  an Ancient Production
  Technology”](https://doi.org/10.1016/B978-0-12-384927-4.00009-9), *Advances
  in Food and Nutrition Research* 63 (2011), pp. 285–306.
- Chiara Sanmartin et al., [“Postharvest Water Loss of Wine Grape: When, What
  and Why”](https://doi.org/10.3390/metabo11050318), *Metabolites* 11 (2021),
  article 318.
- Sara Zenoni et al., [“Rapid dehydration of grape berries dampens the
  post-ripening transcriptomic program and the metabolite profile
  evolution”](https://doi.org/10.1038/s41438-020-00362-5), *Horticulture
  Research* 7 (2020), article 141.
